...
class Crypto {
	static function check_htpasswd_pass($user, $submitted_pass, $pass) {
		if (strpos($pass, '$apr1$') === 0) {
			return APR1_MD5::check($submitted_pass, $pass);
		} else {
			$salt = substr($pass, 0, 2);
			$hashed = crypt($submitted_pass, $salt);
			return $hashed == $pass;
		}
	}
}
